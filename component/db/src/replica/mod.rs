...
use common::{shutdown::Shutdown, OK_FRAME, SYNC_CMD, SYNC_CMD_PING, SYNC_SNAPSHOT};
use dict::Dict;
use tokio::sync::{broadcast, Notify};

use crate::{forward, Db, Role};

/// 发起主从复制
///
/// warn: 这个函数是阻塞的
pub fn process<A: ToSocketAddrs + Clone>(addr: A, db: &Arc<Db>) -> common::Result<()> {
...
/// 向主节点要快照
fn process_snapshot<A: ToSocketAddrs + Clone>(addr: A, db: &Arc<Db>) -> common::Result<()> {
...
fn process_cmd_forward<A: ToSocketAddrs>(
    ...
    db: Arc<Db>,
    notify: Arc<Notify>,
) -> common::Result<broadcast::Sender<()>> {
